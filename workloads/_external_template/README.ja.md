...
| Variable | 値 |
|---|---|
| `WORKLOAD_NAME` | projects-infra に登録されているワークロード名（例: `my-service`）。S3 state キーのプレフィックスとして使用される：`workloads/<WORKLOAD_NAME>/<env>/terraform.tfstate` |
...
- `workloads/<name>/account-bootstrap/terraform.auto.tfvars` に `additional_github_repos = ["org/this-repo"]` が設定済み
