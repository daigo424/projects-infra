...
Template for workloads managed in projects-infra where Terraform code lives in an external repository.
...
For prod-only workloads, remove the `test` entry from the matrix in `terraform-plan.yml` and the `test` case in `terraform-apply.yml`.
